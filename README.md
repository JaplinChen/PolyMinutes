...
| 會議紀錄 | 歷次逐字稿與摘要；在此把 `S1`／`S2` 對應成真實姓名、聽片段、看影片、匯出、刪除、匯入既有錄影 |
| 提問 | 對所有會議一次提問，答案掛回引用的逐字稿句子 |
| 詞彙表 | 專有名詞處理方式 |
...
每一場的逐字稿都可以逐句點擊修改、重跑單行、或攤開翻譯。此外：

**會議摘要**：在該場的逐字稿上按「產生摘要」，LLM 依整場內容分段輸出——決議、行動項目、風險與待確認、未決議題與推測。每一條都掛一個時間碼引用，點下去跳到逐字稿的出處那一句。判斷層（風險／推測）刻意獨立成段，避免把模型的猜測混進事實。摘要走 `llm.json` 設定的供應商，缺金鑰時退到本地 Ollama（見隱私模式）。在「會前資料」欄填議程、與會者或會出現的名詞，重新產生摘要時會併入 prompt。

**影片與片段**：匯入的會議若帶影片，逐字稿每一行旁可播放對應的影片片段——聽聲音認不出誰在說，畫面認得出。影片本身是可拖曳的浮窗，支援子母畫面。

**匯出**：Word（`.docx`）給存檔，WebVTT（`.vtt`）字幕軌讓會議影片在任何播放器都能配字幕。

**提問**：「提問」頁對所有已記錄的會議一次提問（例如「上線日期最後決定了什麼？」），答案的每一句都掛回它引用的逐字稿句子；對不上真實逐字稿的引用會被略去並標示，模型自由發揮的答案不會被當成事實。

### 匯入既有錄影或錄音

在別處錄的會議（Teams 雲端錄影、手機、會議室相機）可在會議紀錄頁匯入，匯入後就是一般的會議紀錄：命名、修正、摘要、提問全部適用。三種來源：

- **上傳檔案**：影片或音訊皆可，音軌用 ffmpeg 抽出
- **貼連結**：http(s) 連結交給 yt-dlp 下載音軌
- **伺服器路徑**：UNC 共享或本機路徑上的錄影直接讀取，不經上傳、只讀不刪

匯入後立即回應，辨識與 LLM 校正排成背景工作在伺服器上跑，進度就顯示在該場上。上傳者若附了手動字幕，直接成稿跳過解碼，音訊仍保留供播放與重跑。
...
|---|---|---|---|
| GPU 即時 | large-v3-turbo float16 | 更快 2.6× | CPU 約 12%，可正常工作 |
| GPU 會後 | large-v3 float16 | 0.15 – 0.30 | CPU 約 12%，可正常工作 |
| CPU | small int8、4 執行緒 | 0.20 | 尚可 |
| CPU | small float32、全部核心 | 0.57 | 整台機器卡死 |

即時路徑用 large-v3-turbo 換取字幕速度；turbo 的語言偵測不可信，靠 per-speaker 鎖定語言補上。會後重跑改用 large-v3 求準。
